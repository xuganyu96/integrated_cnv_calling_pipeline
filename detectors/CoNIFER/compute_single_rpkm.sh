...
echo "[PROGRESS] Computing RPKM values for ${filename_pref} and outputing to ${output_path}"
python ./detectors/CoNIFER/conifer3.py rpkm\
  --probes ./detectors_inputs/CoNIFER/probes.txt \
  --input $BAM_PATH \
  --output $output_path
echo "[COMPLETE] RPKM value computed for ${filename_pref} and output to ${output_path}"